...
#resultsM <- as.data.frame(resultsM[-1,])



# if (nrow(resultsM) > 2) { resultsM <- as.matrix(resultsM[-1,])
	# } else {		
	# resultsM <- matrix(resultsM[-1,], 1, ncol(resultsM))
	# }
	
	
# if (nrow(resultsM) > 2) { resultsM <- as.matrix(resultsM[-1,])
	# } else {
	# df <- as.matrix("", ncol = ncol(resultsM), nrow = 1))
	# df <- resultsM[2,]
	# resultsM <- df		
	# }
...
# # 
# listdf <- data.frame(ID=seq_along(resultsM), var1=seq_along(resultsM), var2=seq_along(resultsM))

# df = data.frame(matrix("", ncol = 3, nrow = 10)


# df = data.frame(matrix("", ncol = ncol(resultsM), nrow = 1))
