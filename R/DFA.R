...
cat('\n\nAppropriate significance tests for controlling the maximum familywise Type I error rate (MFWER)')
cat('\nbased on:')
cat('\n\nBird, K. D., & Hadzi-Pavlovic, D. (2013). Controlling the maximum familywise Type I ')
cat('\nerror rate in analyses of multivariate experiments. Psychological Methods, 19(2), p. 265-280.\n')
...
if (ngroups == 2) {
	...
	for (lupe in 1:length(ttestDVoutput)) {
		...
		BirdOut[,5] <- ifelse( abs(BirdOut[,3]) > TCRIT2, 'significant', 'not signif.')
...
if (ngroups > 2) {
	...
	cat('\nmultiple t tests, then the MANOVA and F tests are irrelevant and the')
	...
	cat('\nthen the F value for the data must be greater than the F-critical value below, and')
	cat('\nthe t value for each comparison must be greater than t2-critical value') 
	cat('\nbelow for a pairwise comparison to be significant.\n\n')

	for (lupe in 1:length(ttestDVoutput)) {
		cat(lsnoms[lupe],'\n')

		decisionF <- ifelse( anovaDVoutput[lupe,2] > FCRIT, 'significant', 'not signif.')
